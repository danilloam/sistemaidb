...
switch($_SESSION['perfil']) { 
    ...
    case "Administrador":
        {
               $igreja_id=isset($_GET['ig']) ? $_GET['ig'] : '';
         $regiao_id=isset($_GET['rg']) ? $_GET['rg'] : '';
        $idmembro=isset($_GET['id']) ? $_GET['id'] : '';
        
$sql=mysqli_query($conecta,"select `p.id`, ` p.dataNascimento`, ` p.data_nascimento`, ` p.datacadastro`, ` p.dataprimeiropassos`, ` p.data_batismo`, ` p.databatismoespiritosanto`, ` p.dataCasamento`, ` p.datafalecimento`, ` p.bairro`, ` p.cep`, ` p.cidade`, ` p.email`, ` p.estado`, ` p.logradouro`, ` p.numero`, ` p.telefone`, ` p.celular1`, ` p.opercel1`, ` p.cel1wp`, ` p.celular2`, ` p.opercel2`, ` p.cel2wp`, ` p.cpf`, ` p.rg`, ` p.orgao`, ` p.nome`, ` p.sobrenome`, ` p.sexo`, ` p.status`, ` p.ecivil`, ` p.escolaridade`, ` p.naturalidade`, ` p.nacionalidade`, ` p.nomemae`, ` p.nomepai`, ` p.conjuge`, ` p.funcao`, ` p.membro`, ` p.facebook`, ` p.instagram`, ` p.falecido`, ` p.foto`, ` p.data_modificacao`, ` p.comentario_pastor`, ` p.comentario_secretaria`, ` p.regiao_id`, ` p.igreja_id`, e.descricao from pessoa p inner join estado e on p.estado=e.id where p.id=$idmembro and p.regiao_id=$regiao_id");
$ver=mysqli_fetch_array($sql);
$sql1=mysqli_query($conecta,"select i.nome, i.logradouro, i.numero, i.bairro, i.cidade, i.cep, i.telefone, e.descricao from igreja i inner join estado e on i.estado_id=e.id where i.id=$igreja_id");
$ver1=mysqli_fetch_array($sql1);
$estado_nome_pessoa=$ver["descricao"];
$estado_nome_igreja=$ver1["descricao"];
...
$sql=mysqli_query($conecta,"elect p.data_nascimento, p.datacadastro, p.dataprimeiropassos, p.data_batismo, p.databatismoespiritosanto, p.dataCasamento, p.datafalecimento, p.bairro, p.cep, p.cidade, p.email, p.estado, p.logradouro, p.numero, p.telefone, p.celular1, p.opercel1, p.cel1wp, p.celular2, p.opercel2, p.cel2wp, p.cpf, p.rg, p.orgao, p.nome, p.sobrenome, p.sexo, p.status, p.ecivil, p.escolaridade, p.naturalidade, p.nacionalidade, p.nomemae, p.nomepai, p.conjuge, p.funcao, p.membro, p.facebook, p.instagram, p.falecido, p.foto, p.data_modificacao, p.comentario_pastor, p.comentario_secretaria, p.regiao_id, p.igreja_id, e.descricao from pessoa p inner join estado e on e.id=p.estado where p.id=$idmembro and p.regiao_id=$regiao_id");
$ver=mysqli_fetch_array($sql);
$sql1=mysqli_query($conecta,"select i.nome, i.logradouro, i.numero, i.bairro, i.cidade, i.cep, i.telefone, e.descricao from igreja i inner join estado e on i.estado_id=e.id where i.id=$igreja_id");
$ver1=mysqli_fetch_array($sql1);
$estado_nome_pessoa=$ver["descricao"];
...
$sql=mysqli_query($conecta,"select p.data_nascimento, p.datacadastro, p.dataprimeiropassos, p.data_batismo, p.databatismoespiritosanto, p.dataCasamento, p.datafalecimento, p.bairro, p.cep, p.cidade, p.email, p.estado, p.logradouro, p.numero, p.telefone, p.celular1, p.opercel1, p.cel1wp, p.celular2, p.opercel2, p.cel2wp, p.cpf, p.rg, p.orgao, p.nome, p.sobrenome, p.sexo, p.status, p.ecivil, p.escolaridade, p.naturalidade, p.nacionalidade, p.nomemae, p.nomepai, p.conjuge, p.funcao, p.membro, p.facebook, p.instagram, p.falecido, p.foto, p.data_modificacao, p.comentario_pastor, p.comentario_secretaria, p.regiao_id, p.igreja_id, e.descricao from pessoa p inner join estado e on e.id=p.estado where p.id=$idmembro and p.igreja_id=$igreja_id");
$ver=mysqli_fetch_array($sql);
$sql1=mysqli_query($conecta,"select i.nome, i.logradouro, i.numero, i.bairro, i.cidade, i.cep, i.telefone, e.descricao from igreja i inner join estado e on i.estado_id=e.id where i.id=$igreja_id");
$ver1=mysqli_fetch_array($sql1);
$estado_nome_pessoa=$ver["descricao"];
		...
		<td colspan=3>
			<center>
					...
					echo mb_strtoupper(utf8_encode($ver1["logradouro"].", Nº ".$ver1["numero"]." - ".$ver1["bairro"]));
									?> </h3>
									<h3><?php 
					echo mb_strtoupper(utf8_encode($ver1["cidade"].", ".$ver1["descricao"]));
									?> </h3>
									<h3><?php 
...
if($ver["ecivil"]==2) { ?>
	...
	<tr>
		...
		<td >
			<?php 
					echo mb_strtoupper(utf8_encode($ver["descricao"]));?>
		</td>
		<td >
			<?php 
